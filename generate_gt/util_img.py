...
def depth_to_mesh_df(depth_im, th, jitter, upsample=0.6, cam_dist=2.0):
    ...
    t.camera.focal_length = 0.035
    t.camera.sensor_width = 0.03059411708155671
    #t.camera.focal_length = 0.035
    #t.camera.sensor_width = 0.02
    ...
    # PTCLD NORMALIZATION, WANT THIS? YES ###
    t.ptcld /=np.max(np.abs(t.ptcld))
    t.ptcld *= 0.5
    # PTCLD NORMALIZATION, WANT THIS?  YES ###
    ...
    tdf_s = tdf
    cnt_s = np.zeros([128, 128, 128])
    
    to_remove = np.where(np.abs(t.ptcld) >= 0.5)[0]
    pt_s = copy.deepcopy(t.ptcld)

    if len(to_remove)>0:
        pt_s = np.delete(pt_s, to_remove, axis=0)

    ids_s = np.floor((pt_s+0.5)*128).astype(int)
    center_s = ((ids_s + 0.5) *1/128) - 0.5
    dist_s = np.linalg.norm(center_s - pt_s, axis=1)
    counts = dict(Counter(tuple(row) for row in ids_s))
    
    arr1 = np.array(list(counts.keys()))
    arr2 = np.array(np.array(list(counts.values())))
    cnt_s[arr1[:,0], arr1[:,1], arr1[:,2]] = arr2
    
    tdf_s[cnt_s!=0] = 0
    np.add.at(tdf_s, (ids_s[:,0], ids_s[:,1], ids_s[:,2]), dist_s) 
    np.divide.at(tdf_s, np.where(cnt_s!=0), cnt_s[cnt_s!=0])
    ''' 
    ### ATTEMPTING VECTORIZATION ###
    specific_dists = []
    for pts in t.ptcld:
        ...
        if ids[0] == 0 and ids[1] == 53 and ids[2] == 62:
            specific_dists.append(dist)

        tdf[ids[0], ids[1], ids[2]] = (tdf[ids[0], ids[1], ids[2]] * n + dist) / (n + 1)
    
        #tdf[ids[0], ids[1], ids[2]] = (tdf[ids[0], ids[1], ids[2]] + dist)

        cnt[ids[0], ids[1], ids[2]] += 1
   
    import pdb; pdb.set_trace()
    '''
    #print('finished computing ptcld', tock-tick)
    return tdf_s
